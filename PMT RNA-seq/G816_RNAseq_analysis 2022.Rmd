...
counts.rna <- dplyr::select(counts.rna, GENE, samples.rna$`Sample Names`) %>% # filtering matrix to only have the samples we care about - G816 samples
  distinct(GENE, .keep_all = T) %>% # get rid of duplicate genes 
  column_to_rownames(var="GENE")
colnames(counts.rna) <- samples

write.csv(counts.rna, file = "G816_RNA-seq_counts.csv")
