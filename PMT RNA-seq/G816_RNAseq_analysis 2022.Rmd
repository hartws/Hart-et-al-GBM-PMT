...
phillips_mes <- read_excel("mmc4.xls", sheet = "MES") %>% pull(`Gene Symbol`) %>% match(genes$gene) %>% genes$ensembl[.] %>% na.omit() %>% as.character()
...
gene.sets1 <- msigdbr(species = "Homo sapiens", category = "C2", subcategory = "CGP") %>% 
...
sets.gsea1 <- rbind(gene.sets1, phillips)
...
gseaplot(em, geneSetID = 1, by = "runningScore", 
         title = bquote(paste(.(em$Description[1]), ": ", "TMZ")))+
  annotation_custom(grobTree(textGrob(bquote(atop(paste("NES: ", .(em$NES[1] %>% round(2))), 
                                       paste("p-value: ", .(em$pvalue[1] %>% round(4))))), 
                           x=0.7,  y=0.8, hjust=0,
  gp=gpar(col="red", fontsize=8))))+
  plot_theme
ggsave("C:/Users/willh/Documents/Lab/GBM PMT/PMT manuscript/NES_1_TMZ.svg")
ggsave("C:/Users/willh/Documents/Lab/GBM PMT/PMT manuscript/NES_1_TMZ.png", width = 4, height = 2, units = "in")


gseaplot(em, geneSetID = 2, by = "runningScore", 
         title = bquote(paste(.(em$Description[2]), ": ", "TMZ")))+
  annotation_custom(grobTree(textGrob(bquote(atop(paste("NES: ", .(em$NES[2] %>% round(2))), 
                                       paste("p-value: ", .(em$pvalue[2] %>% round(4))))), 
                           x=0.7,  y=0.8, hjust=0,
  gp=gpar(col="red", fontsize=8))))+
  plot_theme
ggsave("C:/Users/willh/Documents/Lab/GBM PMT/PMT manuscript/NES_2_TMZ.svg")
ggsave("C:/Users/willh/Documents/Lab/GBM PMT/PMT manuscript/NES_2_TMZ.png", width = 4, height = 2, units = "in")


gseaplot(em, geneSetID = 3, by = "runningScore", 
         title = bquote(paste(.(em$Description[3]), ": ", "TMZ")))+
  annotation_custom(grobTree(textGrob(bquote(atop(paste("NES: ", .(em$NES[3] %>% round(2))), 
                                       paste("p-value: ", .(em$pvalue[3] %>% round(4))))), 
                           x=0.7,  y=0.8, hjust=0,
  gp=gpar(col="red", fontsize=8))))+
  plot_theme
ggsave("C:/Users/willh/Documents/Lab/GBM PMT/PMT manuscript/NES_3_TMZ.svg")
ggsave("C:/Users/willh/Documents/Lab/GBM PMT/PMT manuscript/NES_3_TMZ.png", width = 4, height = 2, units = "in")


gseaplot(em, geneSetID = 4, by = "runningScore", 
         title = bquote(paste(.(em$Description[4]), ": ", "TMZ")))+
  annotation_custom(grobTree(textGrob(bquote(atop(paste("NES: ", .(em$NES[4] %>% round(2))), 
                                       paste("p-value: ", .(em$pvalue[4] %>% round(4))))), 
                           x=0.7,  y=0.8, hjust=0,
  gp=gpar(col="red", fontsize=8))))+
  plot_theme
ggsave("C:/Users/willh/Documents/Lab/GBM PMT/PMT manuscript/NES_4_TMZ.svg")
ggsave("C:/Users/willh/Documents/Lab/GBM PMT/PMT manuscript/NES_4_TMZ.png", width = 4, height = 2, units = "in")
...
library(ggupset)
pathway_genes <- data.frame(Pathway = "Verhaak MES", gene = unlist(verhaak.mes)) %>% 
  rbind(data.frame(Pathway = "Verhaak PN", gene = unlist(verhaak.pn))) %>% 
  rbind(data.frame(Pathway = "Phillips MES", gene = unlist(phillips_mes))) %>% 
  rbind(data.frame(Pathway = "Phillips PN", gene = unlist(phillips_pn))) %>% 
  rbind(data.frame(Pathway = "Segerman", gene = unlist(segerman_mes))) %>% 
  rbind(data.frame(Pathway = "Hallmark TGFb", gene = unlist(hall.tgfb))) %>% 
  rbind(data.frame(Pathway = "Hallmark TNFa", gene = unlist(hall.tnfa)))

pathway_genes <- pathway_genes %>% group_by(Pathway) %>% distinct(gene)

pathway_member <- pathway_genes %>% group_by(gene) %>% summarise(Pathways = list(Pathway))
# pathway_member

pathway_member %>% ggplot(aes(x = Pathways))+
  geom_bar()+
